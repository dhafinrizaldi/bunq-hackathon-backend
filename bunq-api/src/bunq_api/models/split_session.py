...
class User(db.Model):
    ...
    split_sessions = db.relationship("SplitSession", back_populates="user")  # one-to-many
...
class SplitSession(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)  # no unique=True
...
class Receipt(db.Model):
    ...
    split_session_id = db.Column(db.Integer, db.ForeignKey("split_session.id"), unique=True, nullable=False)
