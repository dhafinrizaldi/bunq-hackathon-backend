...
async def make_bunq_request(url: str, method='get', payload=None) -> dict[str, Any] | None:
    ...
    async with httpx.AsyncClient() as client:
        try:
            if method == 'get':
                ...
                response = await client.post(url, json=payload, timeout=30.0, follow_redirects=True)
...
@mcp.tool()
async def create_payment(amount: str, description: str, counterparty_alias: str, counterparty_type: str = "EMAIL") -> str:
    ...
    """
    logger.info("Tool called: create_payment amount=%s counterparty=%s", amount, counterparty_alias)
    payment_data = {
        "amount": amount,
...
@mcp.tool()
async def create_request_inquiry(amount: str, description: str, counterparty_alias: str, counterparty_type: str = "EMAIL") -> str:
    ...
    Args:
        ...
        counterparty_type: Type of alias - "EMAIL", "IBAN", or "PHONE_NUMBER" (default: EMAIL)
    """
    logger.info("Tool called: create_request_inquiry amount=%s counterparty=%s", amount, counterparty_alias)
    request_inq_data = {
        "amount": amount,
        "description": description,
        "counterparty_alias": counterparty_alias,
...
def format_request_inquiry(inquiry: dict) -> str:
    ...
    counterparty = inquiry.get("_counterparty_alias", {}).get("label_monetary_account", {}).get("_display_name", "Unknown")
...
"""
@mcp.tool()
async def send_payment_by_name(recipient_name: str, amount: str, description: str = "") -> str:
    ...
    """
    logger.info("Tool called: send_payment_by_name recipient=%s amount=%s", recipient_name, amount)
    ...
    for payment in payments:
        counterparty = payment.get("_counterparty_alias", {}).get("label_monetary_account", {})
    ...
    # Now create payment
    return await create_payment(amount, description or f"Payment to {recipient_name}", recipient_iban, "IBAN")
...
@mcp.tool()
async def send_request_inq_by_name(recipient_name: str, amount: str, description: str = "") -> str:
    ...
    Args:
        ...
        description: Optional description
    """
    logger.info("Tool called: send_request_inq_by_name recipient=%s amount=%s", recipient_name, amount)
    # Get payments to find recipient
    url = f"{BUNQ_API_BASE}/payments"
    payments = await make_bunq_request(url)
    ...
    # Now create payment
    return await create_request_inquiry(amount, description or f"Payment request to {recipient_name}", recipient_iban, "IBAN")
...
@mcp.tool()
async def split_receipt_by_names(recipient_names: list[str], recepient_splits: list[UserSplit] , receipt: Receipt):
    """
